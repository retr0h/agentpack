...
1. **Build** reads `agentpack.yaml`, resolves content, and produces a generic
   `.agentpack` archive organized by content type.
...
The `GitFetcher` delegates to [gilt](https://github.com/retr0h/gilt) for clone
caching and version checkout via `gilt.Overlay()`. When gilt moves to pure Go
git, agentpack becomes fully git-binary-free.
...
`DefaultInstaller` (wraps `install.Run`). Tests inject `MockBuilder` and
`MockInstaller` generated by `go.uber.org/mock/mockgen`.

## Content types

Archives are generic — organized by type, not by platform. No binary executables
