...
- Safety classification should be a first-class package property, not a
  runtime scan
...
Classification happens at **build time** and is embedded in the package
metadata (`.agentpack/metadata.json`). It is checksummed alongside all
other content. No scanning happens at install time — the installer reads
the classification from trusted, verified metadata.
...
Binary files are rejected at build time — they never appear in metadata
...
no extension could be an ELF binary. Magic bytes:
...
- Safety is a first-class package property — inspectable, checksummed,
  tracked
- Build fails fast on binaries — they never enter the ecosystem
- Users are informed about executable content before it touches their
  system
