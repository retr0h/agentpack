...
agentpack. ADRs capture significant design decisions along with their
...
Additional sections (e.g. **Influences**) are permitted when they add
clarity.
...
- ADRs are point-in-time records — don't rewrite accepted ADRs. When a
  decision is reversed, write a new ADR and mark the old one as
  superseded in its Status section.
